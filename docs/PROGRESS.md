...
- [ ] 家元相談所連携: 友だちごとのメッセージ回数、タグ、最終会話日時、相談カテゴリ、家元モード利用回数、画像・デザインモード利用回数をAPIで返し、家元botの親密度・常連感に使う

### 家元相談所連携メモ (2026-07-09)
- 家元botは商品販売を主目的にせず、親方・師匠として相談を受けるLINE体験。
- 親密度は家元bot単体の推測ではなく、LINE Harnessの友だち情報を正とする。
- 使う情報: メッセージ回数、初回/リピーター、最終会話日時、タグ、シナリオ到達状況、家元相談モード利用回数、画像・デザインモード利用回数、過去の相談カテゴリ。
- 家元bot側はLINE Harness由来の回数・タグ・履歴に直近会話要約を足して返答距離を調整する。
- 例: 初回は「今日はどうした？」、再訪は「また来たな」、常連は「お前の迷い方は少し分かってきた」のように距離感を変える。
- 口コミレビュー職人は別LINE運用。家元相談所のLINE入口からは転送しない。
